...
public class PostRoomsDao {
    ...
    public ArrayList<PostRooms> getAllPostRooms(String email) {
        ...
        try {
            ...
            String sql = "select r.image,room_name,r.price, p.number_house, p.street, p.ward, p.district, p.city , r.position_id from Rooms as r, Users as u , Positions as p where u.user_id= r.user_id and r.position_id= p.position_id and u.email=?";
    ...
    public void updatePostRoom(String room_name, float price , String image,String id){
        ...
        ResultSet rs= null;
        try{
            String sql = "SET room_name = ?, price=? , image=?  WHERE position_id = ?";
            con=database.getConnection();
            pr=con.prepareStatement(sql);
            pr.setString(1, room_name);
            pr.setFloat(2, price);
